...
const liveEnabled = HAS_KEY && process.env.OPENCLAW_LIVE_TEST === "1";
const describeLive = liveEnabled ? describe : describe.skip;

describe("memory-zvec plugin e2e", () => {
  let tmpDir: string;
  let dbPath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-memory-test-"));
    dbPath = path.join(tmpDir, "zvec");
  });
  ...
  test("memory plugin registers and initializes correctly", async () => {
    ...
    expect(memoryPlugin.id).toBe("memory-zvec");
    expect(memoryPlugin.name).toBe("Memory (zvec)");
...
// Live tests that require OpenAI API key and actually use zvec
describeLive("memory-zvec plugin live tests", () => {
  let tmpDir: string;
  let dbPath: string;
